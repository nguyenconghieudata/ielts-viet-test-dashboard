...
export default function Reading() {
  ...
  const init = async () => {
    try {
      ...
      if (res && res?.data?.length > 0) {
        ...
        render(res.data);
        await fetchQuestionCounts(res.data); // Fetch question counts after rendering
      } else {
  ...
  const fetchQuestionCounts = async (items: any) => {
    const counts: Record<string, number> = {};
    for (const item of items) {
      if (item.parts && Array.isArray(item.parts)) {
        try {
          const totalQuestions = await countQuestions(item.parts);
          counts[item.id || item.name] = totalQuestions; // Use a unique identifier
        } catch (error) {
          console.error(
            `Error counting questions for item ${item.name}:`,
            error
          );
  ...
  const countQuestions = async (parts: any) => {
    ...
    for (const part of parts) {
      try {
        // Assuming parts contains IDs for questions
        const response = await QuestionsService.getQuestionsById(part);
        const questionCount = response?.question?.length || 0;
        console.log(`Questions for part ${part}:`, questionCount);
        total += questionCount;
      } catch (error) {
        console.error(`Error fetching questions for part ${part}:`, error);
      }
    }
