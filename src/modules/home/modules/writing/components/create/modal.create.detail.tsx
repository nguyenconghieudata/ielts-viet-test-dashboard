...
export function ModalCreateWritingDetail({
  ...
  const replaceBase64WithCloudUrls = async (
    ...
    const imgTagRegex =
      /<img[^>]+src=["'](data:image\/[^;]+;base64[^"']+)["'][^> Sari]*>/g;
  ...
  const handleMainImageChange = (
    ...
    reader.onloadend = () => {
      ...
      const updatedParts = parts.map((part) =>
        part.part_num === activePart ? { ...part, image: imageData } : part
      );
      onPartsUpdate(updatedParts);
    };
  ...
  const handleSaveQuestions = async () => {
    ...
    try {
      ...
      for (const part of updatedParts) {
        ...
        // Create a new question from image and content
        ...
        // Update questions array, replacing existing questions
        updatedParts = updatedParts.map((p) =>
          p.part_num === part.part_num
            ? {
                ...
                image: partImageUrl,
                content: partContent,
                questions: [newQuestion], // Replace with new question
                tempQuestions: [], // Clear tempQuestions
              }
            : p
        );
      }

      // Update parent state and reset previews
      onPartsUpdate(updatedParts);
  ...
  return (
    <Dialog>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            <span className="!text-[20px]">Tạo đề bài</span>
          </DialogTitle>
          <DialogDescription>
